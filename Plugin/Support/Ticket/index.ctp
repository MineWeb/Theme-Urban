<div id="heading-breadcrumbs">
  <div class="container">
                <h2 style="margin-top: 40px;margin-bottom: 60px;font-size:45px;text-transform: uppercase;" class="section-title text-center wow fadeInDown">☰=- <?= $Lang->get('SUPPORT') ?> -=☰</h2>
    <div class="row">
      <div class="col-md-8">
        <h1></h1>
      </div>
      <div class="col-md-4">
        <?php if($isConnected AND $Permissions->can('POST_TICKET')) { ?>
          <button data-toggle="modal" data-target="#post_ticket" style="margin-top:15px;" class="btn-post-ticket btn btn-template-main transparent btn-block"><icon class="fa fa-pencil-square-o"></icon> <?= $Lang->get('POST_A_TICKET') ?></button>
        <?php } ?>
        </br>
      </div>
    </div>
            ...
            <?php foreach ($tickets as $key => $value) { ?>
              <?php if($Permissions->can('VIEW_TICKETS') AND $value['Ticket']['private'] == 0 OR $isConnected AND $user['isAdmin'] OR $isConnected AND $user['pseudo'] == $value['Ticket']['author'] OR $Permissions->can('VIEW_ALL_TICKETS')) { ?>
                  ...
                  <div class="col-md-12" id="ticket-<?= $value['Ticket']['id'] ?>">
                    <div class="content-ticket panel panel-primary">
                      <div class="panel-body">
                        <h3 style="font-family: 'Montserrat Subrayada', sans-serif;" class="support">
                          <?= $value['Ticket']['title'] ?> <?php if($value['Ticket']['state'] == 1) { echo '<icon style="color: green;" class="fa fa-check" title="'.$Lang->get('RESOLVED').'"></icon>'; } else { echo '<div style="display:inline-block;" id="ticket-state-'.$value['Ticket']['id'].'"><icon class="fa fa-times" style="color:red;" title="'.$Lang->get('UNRESOLVED').'"></icon></div>'; } ?>
                            <span class=""><small><?= $value['Ticket']['author']?></small></span>
                        </h3>
                        <img style="float:left;" class="support" src="<?= $this->Html->url(array('controller' => 'API', 'action' => 'get_head_skin', 'plugin' => false, $value['Ticket']['author'], 80)) ?>" title="<?= $value['Ticket']['author'] ?>">
                        <div style="float:right;" class="btn-agency-support">
                        ...
                        <p style="margin-left:100px;" class="support"><?= $value['Ticket']['content'] ?></p>
                      </div>
                    </div>
                        ...
                        <?php foreach ($reply_tickets as $k => $v) { ?>
                          ...
                          <div id="ticket-reply-<?= $v['ReplyTicket']['id'] ?>">
                            ...
                            <div class="col-md-11 reply-col">
                              <div class="content-ticket panel panel-primary">
                                <div class="panel-body">
                                  <img style="float: left;" class="support" src="<?= $this->Html->url(array('controller' => 'API', 'action' => 'get_head_skin/', 'plugin' => false)) ?>/<?= $v['ReplyTicket']['author']; ?>/50" title="<?= $v['ReplyTicket']['author']; ?>">
                                  <?php if($isConnected AND $user['isAdmin']) { ?>
                                  <div>
                                    <p style="float:right;"><button id="<?= $v['ReplyTicket']['id'] ?>" title="<?= $Lang->get('GLOBAL__DELETE') ?>" class="btn btn-danger btn-sm reply-delete"><icon class="fa fa-times"></icon></button></p>
                                </div>
                                <?php } ?>
                                  <p style="margin-left:60px;" class="support"><?= before_display($v['ReplyTicket']['reply']) ?></p>
                                </div>
                              </div>
                            </div>
                          </div>
